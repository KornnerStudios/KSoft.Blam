...
namespace KSoft.Blam.Megalo.Model
{
	...
	sealed partial class MegaloScriptModelDecompilerState
	{
		...
		Proto.MegaloScriptDatabase Database => Model.Database;
		Variants.GameEngineMegaloVariant Variant => Model.MegaloVariant;
		...
		void DecompileVariantStringTable()
		{
			...
			{
				Localization.StringTables.LocaleStringTableReference sref = string_table[x];
		...
		{
			Localization.StringTables.LocaleStringTable string_table = Variant.StringTable;

			foreach (Variants.MegaloVariantPlayerTraitsBase obj in Variant.PlayerTraits)
			{
		...
		void DecompileVariantUserDefinedOptions()
		{
			...
			{
				Variants.MegaloVariantUserDefinedOption obj = Variant.UserDefinedOptions[x];
				if (obj.NameStringIndex.IsNotNone() &&
		...
		{
			foreach (MegaloScriptUnionGroup group in Model.UnionGroups)
			{
		...
		public void DecompileGameStatistics()
		{
			...
			foreach (MegaloScriptGameStatistic obj in Model.GameStatistics)
			{
				if (obj.NameStringIndex.IsNotNone())
				{
					obj.CodeName = string_table[obj.NameStringIndex].English;
		...
		void DecompileObjectFilterName(MegaloScriptObjectFilter filter, int filterIndex)
		{
			...
			else
			{
				...
				// silence IDE0059 // Unnecessary assignment of a value
				Util.MarkUnusedVariable(ref prefix_underscore);
			}
		...
		public void DecompileObjectFilters()
		{
			...
			{
				MegaloScriptObjectFilter obj = Model.ObjectFilters[x];
		...
		public void DecompileCandySpawnerFilters()
		{
			...
			{
				MegaloScriptGameObjectFilter obj = Model.CandySpawnerFilters[x];
				if (obj.LabelStringIndex.IsNotNone())
				{
					obj.CodeName = string_table[obj.LabelStringIndex].English;
				}
		...
		void DecompileVariableNames(MegaloScriptModelVariableSet set)
		{
			...
			{
				MegaloScriptCustomVariable variable = set.Numerics[x];
				Proto.MegaloScriptProtoVariableReferenceMember custom_var_proto_member = custom_var_proto_type.Members[variable.Var.Type];
				Proto.MegaloScriptValueType value_type = custom_var_proto_member.ValueType;
			...
			{
				MegaloScriptTimerVariable variable = set.Timers[x];
				Proto.MegaloScriptProtoVariableReferenceMember custom_var_proto_member = custom_var_proto_type.Members[variable.Var.Type];
				Proto.MegaloScriptValueType value_type = custom_var_proto_member.ValueType;
			...
			{
				MegaloScriptTeamVariable variable = set.Teams[x];
			...
			{
				MegaloScriptPlayerVariable variable = set.Players[x];
				variable.CodeName = string.Format(Util.InvariantCultureInfo,
					"{0}{1}{2}", prefix, "Player", x.ToString(Util.InvariantCultureInfo));
			}
			for (int x = 0; x < set.Objects.Count; x++, sb.Clear())
			{
				MegaloScriptObjectVariable variable = set.Objects[x];
				variable.CodeName = string.Format(Util.InvariantCultureInfo,
