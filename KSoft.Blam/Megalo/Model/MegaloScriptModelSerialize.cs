	...
	partial class MegaloScriptModel
	{
			...
			where TCursor : class
		{
			if (objType == MegaloScriptModelObjectType.Condition && TagElementStreamSerializeFlags.UseConditionTypeNames())
			{
				...
				if (s.IsReading)
				{
					if (Database.TryGetCondition(name, out Proto.MegaloScriptProtoCondition proto))
					{
						type = proto.DBID;
					}
					else
			...
			else if (objType == MegaloScriptModelObjectType.Action && TagElementStreamSerializeFlags.UseActionTypeNames())
			{
				...
				if (s.IsReading)
				{
					if (Database.TryGetAction(name, out Proto.MegaloScriptProtoAction proto))
					{
						type = proto.DBID;
					}
					else
		...
		protected void SerializeGameObjectFilters<TDoc, TCursor>(IO.TagElementStream<TDoc, TCursor, string> s)
			...
			where TCursor : class
		{
			using (var bm = s.EnterCursorBookmarkOpt("GameObjectFilters", CandySpawnerFilters, Predicates.HasItems))
			{
				if (bm.IsNotNull)
				{
			...
			where TCursor : class
		{
			...
			using (s.EnterOwnerBookmark(this))
			{
				...
				using (var bm = s.EnterCursorBookmarkOpt("ObjectTypeReferences", ObjectTypeReferences, Predicates.HasBits))
				{
					if (bm.IsNotNull)
					{
				...
				using (var bm = s.EnterCursorBookmarkOpt("Statistics", GameStatistics, Predicates.HasItems))
				{
					if (bm.IsNotNull)
					{
				...
				#region HudWidgets
				using (var bm = s.EnterCursorBookmarkOpt("HudWidgets", HudWidgets, Predicates.HasItems))
				{
					if (bm.IsNotNull)
					{
				...
				using (var bm = s.EnterCursorBookmarkOpt("ObjectFilters", ObjectFilters, Predicates.HasItems))
				{
					if (bm.IsNotNull)
					{
				...
				#region Variables
				static bool set_is_not_empty(MegaloScriptModelVariableSet v) => v.IsNotEmpty;

				using (var bm = s.EnterCursorBookmarkOpt("GlobalVariables", GlobalVariables, set_is_not_empty))
				{
					if (bm.IsNotNull)
					{
				...
				using (var bm = s.EnterCursorBookmarkOpt("PlayerVariables", PlayerVariables, set_is_not_empty))
				{
					if (bm.IsNotNull)
					{
				...
				using (var bm = s.EnterCursorBookmarkOpt("ObjectVariables", ObjectVariables, set_is_not_empty))
				{
					if (bm.IsNotNull)
					{
				...
				using (var bm = s.EnterCursorBookmarkOpt("TeamVariables", TeamVariables, set_is_not_empty))
				{
					if (bm.IsNotNull)
					{
				...
				using (var bm = s.EnterCursorBookmarkOpt("ExecutionOrder", TriggerExecutionOrder, Predicates.HasItems))
				{
					if (bm.IsNotNull)
					{
