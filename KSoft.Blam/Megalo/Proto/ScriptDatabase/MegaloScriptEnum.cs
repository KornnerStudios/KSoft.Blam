...
namespace KSoft.Blam.Megalo.Proto
{
		...
		: IO.ITagElementStringNameStreamable
	{
		...
		{
			MegaloScriptEnum e = db.Enums[enumValueType.EnumIndex];
			if (e.NameToIndex.TryGetValue(name, out int index))
			{
		...
		internal readonly struct EnumNameResolvingContext
		{
		...
		internal static uint FromFlagsName(MegaloScriptDatabase db, MegaloScriptValueType flagsValueType, string names)
		{
			...
			{
				if (e.NameToIndex.TryGetValue(parts[x], out int bit))
				{
					flags |= 1U << bit;
				}
				else
		...
		internal readonly struct FlagsNameResolvingContext
		{
			readonly MegaloScriptDatabase Db;
			readonly MegaloScriptValueType ValueType;
